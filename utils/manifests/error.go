...
func ErrUnmarshalSyntax(err string, packageName string, manifest string) error {
	return errors.New(ErrUnmarshalSyntaxCode, errors.Alert, []string{"Unmarshal syntax error."}, []string{fmt.Sprintf("helm chart of %s has syntax error(s): \n%s \n %s", packageName, err, manifest)}, []string{"Invalid object format"}, []string{"Make sure to input a valid YAML object"})
}
